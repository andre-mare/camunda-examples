...
IP=$(curl --location --fail --silent --request POST 'http://localhost:8080/engine-rest/task' --header 'Content-Type: application/json' --data-raw '{ "processInstanceBusinessKey": "process-key-123", "taskDefinitionKey": "user-task-2a"}' | jq -r '.[0].id')
